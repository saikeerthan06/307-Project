...
#     Robust to:
#       - work[c] returning a DataFrame (if c was a list)
#       - missing columns
#       - unseen / missing categories
#       - string/yes-no casing
#     """
#     work = df.copy()
#     work.columns = [str(c) for c in work.columns]

#     def _series(colname: str) -> pd.Series:
#         """Always return a Series; if DataFrame slipped in, take the first col (and error if >1)."""
#         obj = work[colname]
...
#     for col, mapping in enc.get("binary", {}).items():
#         if col in work.columns:
#             s = _series(col).astype(str).str.strip().str.lower()
#             m = {str(k).strip().lower(): int(v) for k, v in dict(mapping).items()}
#             work[col] = s.map(m).astype("Int64")
...
#     # -------- One-hot features --------
#     #  info can be either a list of categories OR {"categories": [...]}.
#     for col, info in enc.get("onehot", {}).items():
...
#                 return df
#             try:
#                 # If all expected columns are present (and in correct order), use the scaler directly
#                 if set(use) == set(cols):
#                     df.loc[:, cols] = scaler.transform(df[cols])
#                 else:
#                     # Partial overlap: apply scaling per-column using learned stats to avoid shape errors
...
#                     else:
#                         # Fallback: skip scaling when the shape mismatches
#                         pass
#             except Exception:
